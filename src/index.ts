...
const RevealGamepadController = ((Reveal: any) => {
    ...
    return {
        ...
        init: (reveal: any) => {
            let config = {
                loopTime: 50,
                mapping: MAPPINGS['SN30_8bitDo'],
                ...reveal.getConfig().gamepadController
            };
            let interval = -1;
            ...
            let previousButtonsKey: string[] = [];

            function pollGamepads(mapping: Mapping) {
                const gamepads = navigator.getGamepads();
                for (const gp of gamepads.filter(gp => !!gp) as Gamepad[]) {
                    // Find buttons that are currently pressed
                    const pressed = (gp?.buttons ?? []).map((button, idx) => button.pressed ? ({
                        idx,
                        // @ts-ignore
                        ...mapping.buttons.find(mappingButton => mappingButton.idx === idx)
                    }) : null).filter(but => !!but) as Button[];

                    // Find all current crosses orientation
                    let crossValues: CrossAction[] = [];
                    mapping.cross.forEach(cross => {
                        const crossPadAxeValue = gp.axes[cross.idx];

                        // Search closest value to determine cross orientation
                        let min = 99999;
                        let crossAction: CrossAction | null = null;
                    ...
                    const allPressedButtonsKeys = [
                        ...pressed.map(button => button?.name ?? `Undefined index in button mapping pressed: ${button?.idx}` ?? 'No button defined'),
            ...
            window.addEventListener("pressed", (evt: any) => {
